...
chn = sample(dcanalysis(elements,nodes,voltages,amountOfMeasurements,length(observedNodeIndices),measurementMatrix,observedNodeIndices), NUTS(0.65), 10000)
...
hexbin(R1,R2,nbins=30, background_color=:transparent)
xlabel!("R1 [Ω]")
ylabel!("R2 [Ω]")
savefig("divider_sampling_hexbin.svg")

using Serialization
f = open("measurements/spannungsteiler.divider_chain.jls", "w")
serialize(f,chn)
close(f)
weights = exp.(chn[:lp]);
...
# Get estimate via mean

estimateR1 = mean(chn[Symbol("elementsR[\"R1\"].R")])
println(estimateR1)
estimateR2 = mean(chn[Symbol("elementsR[\"R2\"].R")])
println(map_estimateR2)